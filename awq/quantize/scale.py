...
@torch.no_grad()
def apply_clip(module, name, max_val):
...
def apply_scale(module, scales_list, input_feat_dict=None):
    for prev_op_name, layer_names, scales in scales_list:
        ...
        if (
            (isinstance(prev_op, nn.Linear) or prev_op.__class__.__name__ == "FP8Linear")
            and type(layers) == list
            and (isinstance(layers[0], nn.Linear) or layers[0].__class__.__name__ == "FP8Linear")
        ):
            scale_fc_fcs(prev_op, layers, scales)

        elif isinstance(prev_op, nn.Linear) or prev_op.__class__.__name__ == "FP8Linear":
            assert len(layers) == 1
