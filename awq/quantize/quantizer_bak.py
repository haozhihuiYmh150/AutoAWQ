...
def weight_dequant(x: torch.Tensor, s: torch.Tensor, y: torch.Tensor = None, block_size: int = 128) -> torch.Tensor:
...
def dq_fp8_weight(named_linears, layer_id):
    # set dq weight
    logging.info(f'Start dq layer [{layer_id}]')
    orig_weights = {}
...
class AwqQuantizer:
    ...
    def quantize(self):
        for i in tqdm(range(len(self.modules)), desc="AWQ"):
            ...
            with dq_fp8_weight(named_linears, i):
                # [STEP 2]: Compute and apply scale list
                print(f'get_layers_for_scaling')
                module_config: List[Dict] = self.awq_model.get_layers_for_scaling(
                    self.modules[i], input_feat, self.module_kwargs
                )
                print(f'_search_best_scale')
                scales_list = [
    ...
    def _compute_best_scale(
        ...
        org_sd = {k: v for k, v in module2inspect.state_dict().items()}
    ...
    def _search_best_clip(self, layer, named_linears, input_feat):
        ...
        print()
        for i, name in enumerate(named_linears):
            # due to qk bmm, it is hard to clip precisely
            if any([_ in name for _ in avoid_clipping]):
                continue
            # work_rank = "cuda:" + str(i % torch.cuda.device_count())
            # raw_rank = named_linears[name].weight.device
            # print(f'compute_best_clip : {name}')
            # named_linears[name].weight = named_linears[name].weight.to(work_rank)
            # input_feat[name] = input_feat[name].to(work_rank)

            max_val = self._compute_best_clip(
                named_linears[name].weight, input_feat[name]
            )

            # named_linears[name].weight = named_linears[name].weight.to(raw_rank)
            # input_feat[name] = input_feat[name].to(raw_rank)
    ...
    def _compute_best_clip(
        ...
        oc_batch_size = 1024 if org_w_shape[0] % 1024 == 0 else 64  # prevent OOM
